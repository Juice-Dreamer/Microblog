...
from blog_app.main.forms import EditProfileForm, PostForm, EmptyForm
...
def follow(username):
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            flash(_l('User %(username)s not found.', username))
        ...
        db.session.commit()
        flash(_l('Successfully followed %(username)s.', user.username))
        return redirect(url_for('main.user', username=username))
    else:
        return redirect(url_for('main.index'))
...
def unfollow(username):
    ...
    if form.validate_on_submit():  # 只能验证submit提交的post请求
        ...
        db.session.commit()
        flash(_l('Successfully unfollowed %(username)s.', user.username))
        return redirect(url_for('main.user', username=username))
    else:
